...
import { getFirestore, collection, getDocs, DocumentReference, setDoc, addDoc, DocumentData, doc, updateDoc, deleteDoc } from 'firebase/firestore';
import { Block, Client, Day, Exercise, ExerciseReference, ExerciseType, Iteration, Max, Week, WorkoutSchedule, WorkoutScheduleState } from "../types/types";
import { deleteSchedule } from "../features/workoutScheduleSlice";
...
export default class DataPersistence {
    ...
    buildDayFromApiResponse(dayData: any) {
        const day: Day = {
            Exercises: []
        }
    ...
    buildWeekFromApiResponse(weekData: any) {
        const week: Week = {
            Days: []
        }
    ...
    buildBlockFromApiReponse(blockData: any) {
        const block: Block = {
            Weeks: []
        }
    ...
    async addNewDay(newDay: Day): Promise<DocumentData> {
        ...
        return addDoc(daysCollection, newDay);
    }
    ...
    async updateSchedules(schedules: WorkoutScheduleState): Promise<void> {
        const db = getFirestore(this.firebaseApp);
        // const schedulesCollection = collection(db, 'Schedules');
        schedules.Schedules.forEach(async (schedule: WorkoutSchedule) => {
